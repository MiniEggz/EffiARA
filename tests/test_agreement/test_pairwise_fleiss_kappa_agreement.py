...
from src.effiara.agreement import (
    pairwise_fleiss_kappa_agreement,
)
...
@pytest.mark.parametrize(
    ...
    [
        # perfect agreement (fleiss' kappa = 1)
        (
            {
        ...
        # partial agreement (fleiss' kappa between 0 and 1)
        (
            {
        ...
        # no agreement at all (fleiss' kappa negative)
        (
            {
                "annotator_1": ["A", "B", "C", "A", "B"],
            ...
            {"A": 0, "B": 1, "C": 2},
            pytest.approx(-0.5151515, rel=1e-4),
        ),
    ],
)
def test_valid_fleiss_kappa_agreement(data, label_mapping, expected_output):
    df = pd.DataFrame(data)
    result = pairwise_fleiss_kappa_agreement(
