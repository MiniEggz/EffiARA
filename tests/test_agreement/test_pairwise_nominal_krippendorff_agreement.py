...
from src.effiara.agreement import (
    pairwise_nominal_krippendorff_agreement,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"A": 0, "B": 1, "C": 2},
            pytest.approx(0.4375, rel=1e-4),
        ),
        # no agreement at all (krippendorff's alpha negative)
        (
            {
                "annotator_1": ["A", "B", "C", "A", "B"],
            ...
            {"A": 0, "B": 1, "C": 2},
            pytest.approx(-0.363636363636363, rel=1e-4),
        ),
    ],
)
def test_valid_krippendorff_agreement(data, label_mapping, expected_output):
    df = pd.DataFrame(data)
    result = pairwise_nominal_krippendorff_agreement(
