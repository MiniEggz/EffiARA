...
import numpy as np
import pandas as pd
from src.effiara.agreement import calculate_krippendorff_alpha_per_label

# valid calculations
@pytest.mark.parametrize("data, expected_output", [
    # perfect agreement (krippendorff's alpha = 1)
    ({
        "annotator_1": [np.array([1, 0, 1]), np.array([0, 1, 0]), np.array([1, 1, 1])],
        "annotator_2": [np.array([1, 0, 1]), np.array([0, 1, 0]), np.array([1, 1, 1])]
    }, 1.0),

    # partial agreement (krippendorff's alpha between 0 and 1)
    ({
        "annotator_1": [np.array([1, 0, 1]), np.array([0, 1, 0]), np.array([1, 1, 1])],
        "annotator_2": [np.array([0, 1, 1]), np.array([1, 0, 0]), np.array([1, 0, 1])]
    }, pytest.approx(0.027777, rel=1e-4)),

    # no agreement (krippendorff's alpha negative)
    ({
        "annotator_1": [np.array([1, 0, 1]), np.array([0, 1, 0]), np.array([1, 1, 1])],
        "annotator_2": [np.array([0, 1, 0]), np.array([1, 0, 1]), np.array([0, 0, 0])]
    }, pytest.approx(-0.66666, rel=1e-4)),

    # lists instead of np.arrays
    ({
        "annotator_1": [[1, 0, 1], [0, 1, 0], [1, 1, 1]],  # Lists instead of numpy arrays
        "annotator_2": [[0, 1, 1], [1, 0, 0], [1, 0, 1]]
    }, pytest.approx(0.027777, rel=1e-4)),
])
def test_valid_krippendorff_alpha_per_label(data, expected_output):
    df = pd.DataFrame(data)
    result = calculate_krippendorff_alpha_per_label(df, "annotator_1", "annotator_2", agreement_type="nominal")
...
@pytest.mark.parametrize("data", [
    ...
    {
        "annotator_1": [np.array([1, 0, 1, 0]), np.array([0, 1, 0, 0]), np.array([1, 1, 1, 1])],
        "annotator_2": [np.array([1, 0, 1]), np.array([0, 1, 0]), np.array([1, 1, 1])]
    }
])
def test_mismatched_annotation_shapes(data):
    ...
    with pytest.raises(ValueError):
        calculate_krippendorff_alpha_per_label(df, "annotator_1", "annotator_2", agreement_type="nominal")
