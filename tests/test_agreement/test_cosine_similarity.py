import pytest
import numpy as np
from src.effiara.agreement import cosine_similarity  # Update the import path as needed

# valid calculations
@pytest.mark.parametrize("vector_a, vector_b, expected_output", [
    ...
    (np.array([1, 2, 3]), np.array([4, 5, 6]), pytest.approx(0.974632, rel=1e-4)),  # General case
    (np.array([0.1, 0.2, 0.3]), np.array([0.4, 0.5, 0.6]), pytest.approx(0.974632, rel=1e-4))  # Small values
])
...
@pytest.mark.parametrize("vector_a, vector_b", [
    (np.array([0, 0]), np.array([1, 1])),  # Zero vector should cause division by zero
    (np.array([1, 1]), np.array([0, 0])),  # Zero vector should cause division by zero
...
@pytest.mark.parametrize("vector_a, vector_b", [
    ...
    ([1, 2, 3], np.array([1, 2, 3])),  # first argument is a list instead of np.ndarray
    (np.array([1, 2, 3]), [1, 2, 3]),  # second argument is a list instead of np.ndarray
