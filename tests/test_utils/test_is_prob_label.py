import pytest
import numpy as np
from src.effiara.utils import is_prob_label  # Update the import path based on your project structure

# testing is_prob_label
# valid
@pytest.mark.parametrize("x, num_classes", [
...
def test_valid_prob_labels(x, num_classes):
    assert is_prob_label(x, num_classes), f"Expected {x} to be a valid probability label"
...
def test_invalid_prob_labels(x, num_classes):
    assert not is_prob_label(x, num_classes), f"Expected {x} to be an invalid probability label"
