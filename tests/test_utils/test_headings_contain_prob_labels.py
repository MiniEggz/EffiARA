...
import numpy as np
import pandas as pd
from src.effiara.utils import headings_contain_prob_labels

# return true
@pytest.mark.parametrize("data, num_classes", [
...
def test_valid_headings_contain_prob_labels(data, num_classes):
    ...
    assert headings_contain_prob_labels(df, "col1", "col2", num_classes), "Expected valid probability labels"

# return false
@pytest.mark.parametrize("data, num_classes", [
    # a single invalid entry (wrong shape)
    ({
        "col1": [np.array([0.2, 0.3]), np.array([0.1, 0.9, 0.0])],  # First entry has 2 elements instead of 3
        "col2": [np.array([0.5, 0.3, 0.2]), np.array([0.3, 0.3, 0.4])]
    }, 3),
    ...
    ({
        "col1": [np.array([0.2, 0.3, 0.5]), np.array([0.1, 0.9, 0.0])],
        "col2": [np.array([0.5, 0.3, 0.2]), [0.3, 0.3, 0.4]]  # List instead of numpy array
    }, 3),
...
def test_invalid_headings_contain_prob_labels(data, num_classes):
    df = pd.DataFrame(data)
    assert not headings_contain_prob_labels(df, "col1", "col2", num_classes), "Expected invalid probability labels"
